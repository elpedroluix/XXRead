...
{
	public class RepositoryStoryHDSBackup : RepositoryHDSBackup, IRepository, IRepositoryStoryHDSBackup
	{
		public RepositoryStoryHDSBackup(IXXReadDatabaseHDSBackup database) : base(database)
		{
			_database = database;
		}
		...
		public async Task<List<Story>> GetStories(string query = "")
		{
			query = query == string.Empty ? "select * from Story" : query;
			List<DTO.Story> stories = new List<DTO.Story>();

			List<SQLiteObjects.Story> dbStories = await _database.GetInstance().QueryAsync<SQLiteObjects.Story>(query);
